...
async def create_pr(
    ...
    pr_title = body.title or f"fix({job.issue_type}): Agentloop auto-fix"
    patch_lines = "\n".join(
    ...
    pr_body = body.body or (
        f"## Root Cause\n\n{job.diagnosis}\n\n"
        f"## Changes\n\n{patch_lines}\n\n"
        f"---\n_Generated by [Agentloop](https://agentloop.io) Code Fix for issue `{job.issue_type}`_"
    )
    ...
    branch_name = f"agentloop-fix/{job.issue_type}-{str(job.id)[:8]}"
    job.pr_url = pr_url
