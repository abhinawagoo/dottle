...
with agentloop.session("research-agent", metadata={"scenario": "loop_test"}) as sid:
    print(f"  Session: {sid}")

    # Iteration 1 — normal first search
    with agentloop.span("llm", "gpt-4o: plan research") as s:
        time.sleep(0.3)
    ...
    with agentloop.span("tool", "search_web", input_args={"query": STUCK_QUERY}) as s:
        time.sleep(0.15)
    ...
    with agentloop.span("llm", "gpt-4o: evaluate results") as s:
        time.sleep(0.3)
    ...
    with agentloop.span("llm", "gpt-4o: refine search") as s:
        time.sleep(0.3)
